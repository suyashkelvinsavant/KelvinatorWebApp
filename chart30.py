...
def chart30():
    # with open('./website/json/stocksData/data30.json') as f:
    #     stData = json.load(f)
    with open('./website/json/stockList/stockList.json') as f:
        stockList = json.load(f)
    sUrls=[]
    for symbol in stockList:
        sUrls.append(f"https://query1.finance.yahoo.com/v7/finance/chart/{symbol}?range=1d&interval=30m")
    ...
    with open('./website/json/stocksData/chart30.json','w')as outfile:
