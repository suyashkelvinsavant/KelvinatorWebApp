...
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for sList in sData:
        for stock in sList['spark']['result']:
            stockChartUrl = f"https://query1.finance.yahoo.com/v7/finance/chart/{stock['symbol']}?range=1d&interval=5m"
    ...
    with open('./website/json/stocksData/spark5.json','w')as outfile:
        json.dump(stData,outfile)
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    return stData
    #"marketPrice":stock['response'][0]['meta']['regularMarketPrice'],
#print(stock['response'][0]['timestamp'][i-1])